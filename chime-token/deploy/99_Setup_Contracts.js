...
module.exports = async ({
  ...
  //Try to Auto-fund VRFConsumer contract
  ...
  //Try to Auto-fund ChimeToken contract

  const ChimeToken = await deployments.get('ChimeToken')
  const chimeToken = await ethers.getContractAt('ChimeToken', ChimeToken.address)

  if (await autoFundCheck(chimeToken.address, networkName, linkTokenAddress, additionalMessage)) {
    await hre.run("fund-link", { contract: chimeToken.address, linkaddress: linkTokenAddress })
  } else {
    log("Then run ChimeToken contract with the following command:")
    log("npx hardhat request-random-number --contract " + chimeToken.address + " --network " + networkName)
  }
  log("----------------------------------------------------")
